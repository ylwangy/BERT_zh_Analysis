...
def get_ngrams(raw, gram):
    ...
    for line in raw:
        for i in range(len(line)):
            if i - li < 0:
                lp = p * int(li - i) + line[:i]
            else:
                lp = line[int(i - li):i]
            if i + ri + 1 > len(line):
                rp = line[i:] + p*int(i + ri + 1 - len(line))
            else:
                rp = line[i:int(i+ri+1)]
            ch = lp + rp
            gram_set.add(ch)
    return gram_set
...
def get_sample_embedding(path, emb, chars, default='unk'):
    ...
    emb_dim = len(list(emb_dic.values())[0])
...
def update_gram_dicts(gram2idx, new_grams):
    ...
    for dic, n_gram in zip(gram2idx, new_grams):
        assert len(list(dic.keys())[0]) == len(n_gram[0])
...
def get_dic(chars, tags):
    ...
    for ch in chars:
        char2index[ch] = idx
        idx += 1
    index2char = {v: k for k, v in list(char2index.items())}
    ...
    for dic_keys in tag2index:
        index2tag[dic_keys] = {v: k for k, v in list(tag2index[dic_keys].items())}
...
def pad_bucket(x, y, bucket_len_c=None):
    ...
    if bucket_len_c is None:
        ...
        logging.info('Number of buckets: ', len(bucket_len_c))
    else:
        idy = 0
        for item in xy:
            max_len = len(item[0][-1])
