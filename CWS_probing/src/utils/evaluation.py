...
if __name__ == '__main__':
   ...
   for opt in opts:
      logging.info(opt)
   if len(args) != 2:
      logging.info(g_sInformation)
      sys.exit(1)
   sCandidate = args[0]
   sReference = args[1]
   boundaries = True
   if not os.path.exists(sCandidate):
      logging.info(("Candidate file %s does not exist." % sCandidate))
      sys.exit(1)
   if not os.path.exists(sCandidate):
      logging.info(("Reference file %s does not exist." % sReference))
      sys.exit(1)
   #
   ...
   if ( lReference and not lCandidate ) or ( lCandidate and not lReference ) :
      logging.info("Warning: the reference and the candidate consists of different number of lines!")
   ...
   logging.info(("Word precision:", word_precision))
   logging.info(("Word recall:", word_recall))

   logging.info(("Tag precision:", tag_precision))
   logging.info(("Tag recall:", tag_recall))

   logging.info(("Word F-measure:", word_fmeasure))
   logging.info(("Tag F-measure:",  tag_fmeasure))
