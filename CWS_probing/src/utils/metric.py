...
def decode_tags(lists):
    outs = []
    out = []
    for idx in range(len(lists)):
        sents = []
        for tagsid in range(len(lists[idx])):
            tag = lists[idx][tagsid]
            if '-' in tag:
                tag = tag.replace('E-', 'I-')
                tag = tag.replace('S-', 'B-')
            else:
...
def get_ner_fmeasure2(golden_lists, predict_lists, label_type="BMES"):
    sent_num = len(golden_lists)
    golden_full = []
    predict_full = []
    right_full = []
    ...
    for idx in range(0,sent_num):
        ...
        all_tag += len(golden_list)
        if label_type == "BMES" or label_type == "BIOES":
            gold_matrix = get_ner_BMES2(golden_list)
            pred_matrix = get_ner_BMES2(predict_list)
        else:
            gold_matrix = get_ner_BIO(golden_list)
            pred_matrix = get_ner_BIO(predict_list)
        # print "gold", gold_matrix
...
def get_ner_BMES2(label_list):
    ...
    for i in range(0, list_len):
        ...
        if begin_label in current_label:
            ...
            if b_idx>0 and (mid_label in prev_tag or begin_label in prev_tag ) :
                res.append(str(b_idx) + '-' + str(i))
                b_idx = -1
            prev_tag = current_label
        elif mid_label in current_label:
            if not (mid_label in prev_tag or begin_label in prev_tag):
                b_idx = -1
            prev_tag = current_label
        else:
            print("error tag")
